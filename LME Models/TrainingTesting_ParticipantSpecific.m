...
%            - Must specify PID and cond to be tested on
%            - Outputs the measured (TR) and predicted ankle torque
%              datasets (TR_pred) for the specified PID and Cond, and saves 
%              them in the directory of LMEDir
% Section 2: Plot individual condition of a specific participant
% Section 3: Plot all conditions of a specific participant
...
formulaTR = "TR ~ " + fixed_AllUWB + " + (1|COND)";
...
fileLoc = fullfile(LMEDir, "Cond_" + string(cond));
save(fullfile(fileLoc, ['Cond_', num2str(cond), '_S', PIDs,'_meas.mat']), 'TR')
save(fullfile(fileLoc, ['Cond_', num2str(cond), '_S', PIDs,'_pred.mat']), 'TR_pred')

delete(f)

beep

%% Section 2: Plot idv PID
figure(1); clf; hold on

PIDDir = "C:\Users\zheng\OneDrive\Desktop\ENGG7291 Data\LME models\Participant dependent\Cond_01";
PID = '02';
...
for i = 1:12
    % Grab data
    cond = condAll(i, :);
    expFileName = dir(fullfile(SDir, ['Cond_', cond, '*_meas.mat']));
    expFile = load(fullfile(SDir, expFileName.name));
    expDat = expFile.TR;

    estFileName = dir(fullfile(SDir, ['Cond_', cond, '*_pred.mat']));
    estFile = load(fullfile(SDir, estFileName.name));
    estDat = estFile.TR_pred;

    % Plot all individual conditions
    bounds = (i - 1) * 100 + 1 : i * 100;

    offset_lm = fitlm(expDat, estDat);
    offset = offset_lm.Coefficients.Estimate(1);
    scale = 1 / offset_lm.Coefficients.Estimate(2);

    Diff(bounds, 1) = abs(expDat - scale * (estDat - offset)) / max(expDat);

    plot(bounds, expDat, 'Linewidth', 3, 'Color', col2)
    plot(bounds, scale * (estDat - offset), '.', 'MarkerSize', 8, 'Color', col1)
    xline(i*100, '--k')
    
    R = corr(expDat, estDat-offset);
    Rsquared(i) = R^2;

    RMSE = sqrt(MSE(expDat, scale*(estDat - offset)));
    NRMSE = RMSE / max(expDat);
    NRMSEAll(i, 1) = NRMSE;
...
expFileName = dir(fullfile(SDir, ['Cond_', maxRsquaredCond, '*_meas.mat']));
expFile = load(fullfile(SDir, expFileName.name));
expDat = expFile.TR;

estFileName = dir(fullfile(SDir, ['Cond_', maxRsquaredCond, '*_pred.mat']));
estFile = load(fullfile(SDir, estFileName.name));
estDat = estFile.TR_pred;
...
bounds = 1:100;
yline(0, '--k', 'Layer', 'bottom')
plot(bounds, expDat, 'Linewidth', 5, 'Color', col2)
plot(bounds, scale * (estDat - offset), '.', 'MarkerSize', 17, 'Color', col1)
